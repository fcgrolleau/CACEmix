...
delta_q <- q_11 - q_00
no_mon_er_res <- sum(delta_q * p_c) / sum(p_c)

# Augmentation term
aug_term <- sum(delta_q - p_c * sum(delta_q) / sum(p_c)) / sum(p_c)
no_mon_er_res_eff <- no_mon_er_res + aug_term
...
return(list(no_mon_er_res = no_mon_er_res, no_mon_er_res_eff = no_mon_er_res_eff, # nolint
            mon_er_res = mon_er_res))
}
...
# Run matching on the propensity score
mit_obj <- matchit(f_eta, data = dat, method = "nearest")

# Create a dataframe with the matched data
mit_df <- match.data(mit_obj)
merged_df <- merge(mit_df[mit_df[z_name] == 1, ],
                   mit_df[mit_df[z_name] == 0, ], by = "subclass")

# Compute the numerator and denominator of the PS matching estimator
merged_df$num_diff <- merged_df[, paste0(yvar_name, ".x")] - merged_df[, paste0(yvar_name, ".y")] # nolint
merged_df$denom_diff <- merged_df[, paste0(t_name, ".x")] - merged_df[, paste0(t_name, ".y")] # nolint

# Compute the PS matching estimator
ps_matching <- with(merged_df, sum(num_diff) / sum(denom_diff))
...
cace_estimates <- cbind(
             ...
             "no_mon_er_res_eff" = er_res$no_mon_er_res_eff,
...
dat <- gen_data(n = 2000, p = p, alpha = .5,
