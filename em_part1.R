...
em_1_fun <- function(dat, xvar_names, z_name, t_name,
                     maxit = 100, epsi = 1e-4,
                     bar = TRUE, verbose = TRUE) {
### EM algorithm to estimate conditional probabilities of c,a,n,d
...
fitted_params <- rep(0, 4 * length(xvar_names))
...
h_1 <- (g_1 * l_1) / (g_1 * l_1 + g_2 * l_2 + g_3 * l_3 + g_4 * l_4)
h_2 <- (g_2 * l_2) / (g_1 * l_1 + g_2 * l_2 + g_3 * l_3 + g_4 * l_4)
h_3 <- (g_3 * l_3) / (g_1 * l_1 + g_2 * l_2 + g_3 * l_3 + g_4 * l_4)
h_4 <- (g_4 * l_4) / (g_1 * l_1 + g_2 * l_2 + g_3 * l_3 + g_4 * l_4)

# M step
h_mat <- as.matrix(cbind(h_1, h_2, h_3, h_4))
...
fit <- tryCatch({
  ...
  warning(paste0("glmnet error at iteration ", i, " (monotonicity ie 4 classes)"))
...
param_mat <- sapply(1:4, function(i) coef(fit)[[i]]@x)
...
if (sum((fitted_params - old_params)^2) / length(fitted_params) < epsi) {
  if (verbose) {
    cat("\nEM no. 1 converged at iteration", i, "\n")
  }
  break
}
}
colnames(param_mat) <- c("C", "A", "N", "D")
